    ...
    steps: [
      {
        ...
        description:
          "Evaluación detallada de factores de riesgo y síntomas con nuestros especialistas en un primer contacto.",
      },
      {
        title: "Estudios Específicos",
        description:
          "Realización in-situ de Electrocardiograma, Eco Doppler y Ergometría, según la necesidad clínica.",
      },
      {
        title: "Plan de Tratamiento",
        description:
          "Análisis de resultados en el mismo día y trazado de un plan de cuidado cardiovascular continuo.",
      },
    ],
  },
  gynecology: {
    ...
    steps: [
      {
        ...
        description:
          "Entrevista médica en un ambiente seguro para tu control anual o seguimiento de síntomas.",
      },
      {
        title: "Estudios Complementarios",
        description:
          "Realización de Papanicolau, Colposcopía y Ecografías con equipamiento de alta definición.",
      },
      {
        title: "Diagnóstico y Prevención",
        description:
          "Entrega de resultados, diagnóstico preciso y pautas de prevención personalizadas.",
      },
    ],
  },
};
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="font-body-lg text-body-lg text-white/80 max-w-2xl mx-auto leading-relaxed">
            Conocé cómo adaptamos nuestro modelo de atención integral a las
            necesidades específicas de cada área médica.
          </p>
        </div>
        ...
        >
          {Object.entries(workflows).map(([key, workflow], colIndex) => (
            <div
              key={key}
              className="bg-white/5 backdrop-blur-md rounded-3xl border border-white/10 p-8 md:p-10 shadow-lg hover:bg-white/10 transition-all duration-300"
            >
              <div className="mb-10 flex items-center gap-4 border-b border-white/10 pb-6">
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg shrink-0 ${workflow.themeClass}`}
                >
                  {workflow.icon}
                </div>
                <h3
                  className={`font-headline-md text-xl md:text-2xl font-bold ${workflow.textClass}`}
                >
                  {workflow.title}
                </h3>
              </div>
              ...
              <div className="relative pl-8 md:pl-10 space-y-10 border-l-2 border-white/10 ml-2">
                {workflow.steps.map((step, index) => (
                  <div key={`${key}-${index}`} className="relative">
                    {/* Timeline Dot */}
                    <div
                      className={`absolute -left-11 md:-left-13 top-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shadow-lg shadow-${workflow.themeClass.split(" ")[0].replace("bg-", "")}/20 ${workflow.themeClass}`}
                    >
                      {index + 1}
                    </div>
                    ...
                    <div>
                      <h4
                        className={`font-headline-md text-lg font-bold mb-2 ${workflow.textClass}`}
                      >
                        {step.title}
                      </h4>
              ...
              <div className="mt-8 pt-8 border-t border-white/10 flex justify-start">
                <a
                  ...
                  className={`inline-flex items-center justify-center px-6 py-3 rounded-2xl font-label-md text-label-md font-bold transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg ${workflow.themeClass}`}
                >
                  Agendar Turno
                </a>
              </div>
